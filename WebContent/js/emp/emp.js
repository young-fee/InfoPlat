...
function getAll(current){	
	$.ajax({
		...
		dataType:'json',
		data:{current:current,eis:1},
		type:'post',
		success:function(mydata){
			$("#empTbody").empty();
			$.each(mydata,function(index,xx){
				if(xx.EId!=null){
					var srt='<tr class="part-tr"><td>'+xx.EId+'</td>';
					
					if(xx.EId==0)
						srt+='<td>'+xx.ETruename+'</td>' ;
					else
						srt+='<td><span id="'+xx.EId+'a" ondblclick=huotrue('+xx.EId+',"'+xx.EId+'a","'+xx.ETruename+'")>'+xx.ETruename+'</span></td>' ;
					
					srt+='<td id="'+xx.EId+'tt">';
					if(xx.EId==0||$userLevel!=-1)
						srt+='<span id="'+xx.EId+'t">'+xx.PName+'</span>';
					else
						srt+='<span id="'+xx.EId+'t" ondblclick=huobu('+xx.EId+',"'+xx.EId+'t",'+xx.TPart.PId+',"'+xx.TPart.PName+'")>'+xx.TPart.PName+'</span>';
					srt+='</td><td>';
					if(xx.EId==0||$userLevel!=-1)
						srt+='<span id="'+xx.EId+'n">'+xx.ELoginname+'</span>';
					else
						srt+='<span id="'+xx.EId+'n" ondblclick=huoname('+xx.EId+',"'+xx.EId+'n","'+xx.ELoginname+'")>'+xx.ELoginname+'</span>';
					srt+='</td><td>';
					if(xx.EId==0)
						srt+='<span id="${x.EId}w">'+xx.EPsw+'</span>';
					else 
					{
						if($userLevel==-1)
							srt+='<span id="'+xx.EId+'w" ondblclick=huopsw('+xx.EId+',"'+xx.EId+'w","'+xx.EPsw+'")>'+xx.EPsw+'</span>';
						if($userLevel==1&&$userID==xx.EId)
							srt+='<span id="'+xx.EId+'w" ondblclick=huopsw('+xx.EId+',"'+xx.EId+'w","'+xx.EPsw+'")>'+xx.EPsw+'</span>';
						if($userLevel==1&&$userID!=xx.EId)
							srt+='<span id="'+xx.EId+'w" ondblclick=huopsw('+xx.EId+',"'+xx.EId+'w","'+xx.EPsw+'")>******</span>';
						if($userLevel==0)
							srt+='<span id="'+xx.EId+'w" ondblclick=huopsw('+xx.EId+',"'+xx.EId+'w","'+xx.EPsw+'")>'+xx.EPsw+'</span>';
					}
					...
					if(xx.EId==0)
						...
						srt+='<span id="'+xx.EId+'s" ondblclick=huosex('+xx.EId+',"'+xx.EId+'s","'+xx.ESex+'")>'+xx.ESex+'</span>';
					srt+='</td><td>';
					if(xx.EImg==null)
					...
					//权限级别
					srt+='</td><td>'+(xx.EAdmin==1?'部门主管':xx.EAdmin==0?'普通操作者':'系统管理员')+'<td>';
					if(xx.EId==0||$userLevel!=-1)
						srt+='<a><span><i class="fa fa-ban fa-lg"></i> 不可操作</span></a>';
					else
						srt+='<a class="uppEmp"><i class="fa fa-edit fa-lg"></i> 修改</a>&nbsp;&nbsp;| <a class="delEmp"><i class="fa fa-user-times	 fa-lg"></i> 离职</a><input type="hidden" value="'+xx.EId+'" />';
					$("#empTbody").append(srt);
				}else{
					$current=xx.current;
					$up=xx.up;
...
function delEmp(){
	$(document).on("click",".delEmp",function() {
		...
		$.ajax({
			type : "post",
			url : "../Client_getClientCountServlet",
			data:'eid='+eid,
			async : true,
			...
			success : (function(msg) {
				if (msg > 0) {
					layer.open( {
						...
						title : "修改员工",
						shadeClose : true,
						content : '../Emp_oneEmpServlet?eid=' + eid,
						skin : 'layui-layer-lan',
						shift: 3, //动画类型
						//创建完成层时回调
						success : function(layero, index) {
...
function huonamechuan(pid, obj, xid) {
	...
	if (mxvalue.length == 0) {
		...
		getIE();
		//先判断当前的是否用户名是否存在
		var url = $webName + "/Emp_allloginnameCountServlet?eloginname=" + ss;
...
function getnameback1() {
	getIE();
	var url = $webName + "/Emp_uppTpartServlet?eid=" + mpid + "&eloginname="
			+ mxvalue + "";
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
	xmlhttp.send();
...
function huopswchuan(pid, obj, xid) {
	...
	if (mxvalue.length == 0) {
		...
		getIE();
		var url = $webName + "/Emp_uppTpartServlet?eid=" + pid + "&epsw=" + er
				+ "";
		var myurl = encodeURI(url);
		xmlhttp.open("post", myurl, true);
		xmlhttp.send();
...
function huotruechuan(pid, obj, xid) {
	...
	if (mxvalue.length == 0) {
		...
		getIE();
		var url = $webName + "/Emp_uppTpartServlet?eid=" + pid + "&etruename=" + er;
		var myurl = encodeURI(url);
		xmlhttp.open("post", myurl, true);
		xmlhttp.send();
...
function sexli(pid, obj, xid) {
	...
	var url = "";
	if (biaoshi == "s") {
		url = $webName + "/Emp_uppTpartServlet?esex=" + zhi + "&eid=" + pid
				+ "";
	}
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
...
function huobu(pid, xid, tid, name) {
	...
	selname = name;
	getIE();
	var url = $webName + "/Emp_deptAllServlet?eid=" + pid ;
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
	xmlhttp.send();
...
function libin(pid, Obj) {
	...
	selname1 = selvalue1;
	getIE();
	var url = $webName + "/Emp_upppartEmpServlet?pid=" + ss + "&eid=" + pid
			+ "";
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
	xmlhttp.send();
