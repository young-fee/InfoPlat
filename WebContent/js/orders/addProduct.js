...
function productYear() {
	$(document).on('change','#productArea',function() {
		...
		var pid = $(this).val();
	    $.ajax( {
			url : '	productyear_getProYear.action',
			dataType : 'json',
			data : {
			   pid:pid
			},
			...
			success : function(mydata) {
				$("#productYear").empty().append("<option value='-10'>-- 请选择 --</option>");
				$.each(mydata, function(index, xx) {
					$("#productYear").append("<option value='"+ xx.YId +"'>" + xx.YName + "</option>");
				});
			}
		});
		$.ajax( {
			url : '	inproduct_getStockNum.action',
			dataType : 'json',
			data : {
			   pid:pid
			},
...
function YPrice() {
	$(document).on('change','#productYear',function() {
		var pid = $(this).val();
		$.ajax( {
			url : '	productyear_getPriceByYid.action',
			dataType : 'json',
			data : {
			   pid:pid
			},
...
function submitOrder()
{
  ...
  $(document).on('click','#btn', function(){
		...
		if($("#productSort").val()==-10||$("#productName").val()==-10||yid==-10||pid==-10){
			...
			var ss="ordersmx.TOrders.RId="+rid+"&ordersmx.TProductyear.YId="+yid+"&ordersmx.TProduct.PId="+pid+"&ordersmx.XCount="+XCount+"&ordersmx.XPrice="+chuPrice+"";
			var i = layer.load(0);
			$.post(" 	ordermx_addOrdermx.action",ss,function(mydata){
					layer.close(i);
				if (mydata >= 1) {
