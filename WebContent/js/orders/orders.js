...
function getoneAll(current) {
	...
	$.ajax( {
		url : "${pageContext.request.contextPath}/order_manyTerm.action",
		type : "post",
		data : {
			ris : $ris,
			cid : cid,
				...
				$.each(mydata,function(index, xx) {
					if (xx.RId !== null) {
						...
						var str="<tr> <td>"
							...
							+ xx.RTime
							+ "</td>  <td>"
							+ rs+"<input type='hidden' class='ris' value="+ xx.RIs + "> </input>"
...
function gettwoAll(current) {
	...
	$.ajax( {
		url : "${pageContext.request.contextPath}/ordermx_getOrderMx.action",
		type : "post",
		data : {
		    oid:$proID,
			current : current
