...
function getAll(current){
	$.ajax({
		url:'../Part_allTpartServlet',
		dataType:'json',
		data:{current:current,pis:1},
		type:'post',
		success:function(mydata){
			$("#partTbody").empty();
			$.each(mydata,function(index,xx){
				if(xx.PId!=null){
					var srt='<tr class="part-tr"><td>'+xx.PId+'</td><td>';
					if(xx.PId==0||$userLevel>-1)
						srt+='<span id="'+xx.PId+'n">'+xx.PName+'</span>';
					else
						srt+='<span id="'+xx.PId+'n" ondblclick=huoname('+xx.PId+',"'+xx.PId+'n","'+xx.PName+'");>'+xx.PName+'</span>';
					srt+='</td><td id="'+xx.PId+'sexs" style="text-align: left">';
					if(xx.PId==0||$userLevel>-1)
						srt+='<span id="'+xx.PId+'n">'+(xx.PRemark==null?'暂无备注':xx.PRemark)+'</span>';
					else
						srt+='<span id="'+xx.PId+'s" ondblclick=huobei('+xx.PId+',"'+xx.PId+'s","'+xx.PRemark+'");>'+xx.PRemark+'</span>';
					srt+='</td><td>';
					if(xx.PId==0||$userLevel!=-1)
						srt+='<a><span><i class="fa fa-ban fa-lg"></i> 不可操作</span></a>';
					else
						srt+='<a class="uppPart"> <i class="fa fa-edit fa-lg"></i>修改</a><input type="hidden" value="'+xx.PId+'" />';
					srt+='</td><td>';
					if(xx.PId==0||$userLevel!=-1)
						srt+='<a><span><i class="fa fa-ban fa-lg"></i> 不可操作</span></a>';
					else
						srt+='<a class="delPart"><i class="fa fa-reply fa-lg"></i> 撤销</a><input type="hidden" value="'+xx.PId+'" />';
					srt+='</td></tr>';	
...
function uppPart(){
	...
	$(document).on("click",".uppPart",function() {
		...
		layer.open( {
			...
			title : "修改部门",
			shadeClose : true,
			content : $webName+'/Part_oneTpartServlet?pid='+pid,
			skin : 'layui-layer-lan',
			shift: 3, //动画类型
			//创建完成层时回调
			success : function(layero, index) {
...
function delPart(){
	...
	$(document).on("click",".delPart",function() {
		...
		$.ajax({
			url:'../Part_getTrueEmpClientCountServlet',
			dataType:'json',
			data:{pid:pid},
			type:'post',
			success:function(mydata){
...
function huonamechuan(pid, obj, xid) {
	...
	if (mxvalue.length == 0) {
		...
		getIE();
		//先判断当前的是否部门名称是否存在
		var url = $webName+"/Part_getNameCountServlet?name="
				+ ss;
...
function getnameback1() {
	getIE();
	var url =$webName+"/Part_uppTpartServlet?pid="
			+ mpid + "&pname=" + mxvalue + "";
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
	xmlhttp.send();
...
function huobeichuan(pid, obj, xid) {
	...
	mxvalue = er;
	getIE();
	var url =$webName+"/Part_uppTpartRemarkServlet?pid="
			+ pid + "&premark=" + er + "";
	var myurl = encodeURI(url);
	xmlhttp.open("post", myurl, true);
	xmlhttp.send();
...
function getBei() {
	if (xmlhttp.readyState == 4 & xmlhttp.status == 200) {
		...
		document.getElementById(mxid).outerHTML = "	<span id=" + mxid + " ondblclick=huobei(" + mpid + ",'" + mxid + "','" + mxvalue + " ')>" + mxvalue + "</span>";
		layer.msg('修改成功！', {
			icon : 6,
			time : 2000
		});
