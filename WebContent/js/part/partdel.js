			...
			if(mydata.length>1){
				$.each(mydata,function(index,xx){
					if(xx.PId!=null){
						...
						srt+='<span>'+xx.PName+'</span>';
						srt+='</td><td style="text-align: left">';
						srt+='<span>'+(xx.PRemark==null?'暂无备注':xx.PRemark)+'</span>';
						srt+='</td><td>';
						if($userLevel!=-1)
							srt+='<a><span><i class="fa fa-ban fa-lg"></i> 不可操作</span></a>';
						else
							srt+='<a class="huiPart"><i class="fa fa-retweet fa-lg"></i> 恢复部门</a><input type="hidden" value="'+xx.PId+'" />';
						srt+='</td></tr>';	
