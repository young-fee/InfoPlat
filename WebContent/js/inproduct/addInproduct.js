	...
	$.ajax( {
		...
		success : function(mydata) {
			...
			$.each(mydata, function(index, xx) {
				$("#sortName").append("<option value=" + xx.PId + ">" + xx.PName + "</option>");
			})
		}
	});
};
	...
	$(document).on('change','#sortName',function() {
		...
		$.ajax( {
			...
			success : function(mydata) {
				...
				$.each(mydata, function(index, xx) {
					$("#productName").append("<option value=" + xx.EId + ">"+xx.ETruename+"(" + xx.ELoginname + ")</option>");
				})
			}
		})
	});
...
function addInproduct() {
	$(document).on("click",".btn", function() {
		...
		if (productid==-10) {
			...
			var mypp = "inproduct.TProduct.PId=" + Pid
				 	 + "&inproduct.TEmp.EId=" + productid
					 + "&inproduct.DNumber=" + DNumber
					 + "&inproduct.DRemark=" + ERemark;
	
			$.post("inproduct_addInProduct.action", mypp, function(mydata) {
				var i = layer.load(0);
				if (mydata == 1) {
