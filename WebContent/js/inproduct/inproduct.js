	...
	$.ajax( {
		...
		success : function(mydata) {
			$.each(mydata, function(index, xx) {
				$("#sort").append("<option value='" + xx.SId + "'>" + xx.SName + "</option>");
			});
			$seachType = 1;
			getAll(1);
		}
...
function chanpin() {
	$(document).on('change','#sort',function() {
		...
		var sortid = $(this).val();
		$.ajax({
			url : 'product_getNameBySid.action',
			dataType : 'json',
			data : 'sid=' + sortid,
			type : 'post',
			success : function(mydata) {
...
function outPro(current){
	$.ajax( {
		url : "outproduct_getOutProduct.action",
		type : "post",
		data : {
			pid : $proID,
			current:current
			...
			if (mydata.length > 1) {
				...
				if (xx.OId !== null) {
						$("#empTbody").append("<tr><td>" 
							+ xx.OId + "</td> <td>"
							+ xx.TProduct.PName + "</td><td>"
							+ xx.TEmpByEId.ETruename + "</td><td>" 
							+ xx.ORemark + "</td><td>" 
							+ xx.ONumber + "</td><td>" 
							+ xx.OTime + "</td></tr>");
					} else {
...
function addInPro() {
	...
	$(document).on("click", ".addku", function() {
		...
		layer.open( {
			...
			title : "商品入库",
			shadeClose : true,
			content : "	product_oneTProduct.action?pid=" + ee,
...
function getoneAll(current) {
	...
	$.ajax( {
		url : "${pageContext.request.contextPath}/product_manyTermForIn.action",
		type : "post",
		data : {
			sid : sid,
			pname : pname,
...
function gettwoAll(current) {
	...
	$.ajax( {
		url : "${pageContext.request.contextPath}/inproduct_getInProduct.action",
		type : "post",
		data : {
			pid : $proID,
			current : current
	...
	//获取总数量
	$.ajax({
		url:"${pageContext.request.contextPath}/inproduct_getStockNum.action",
		type:"post",
		data:{
			pid:$proID,
		},
