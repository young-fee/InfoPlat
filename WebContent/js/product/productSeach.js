...
function getFirstAll(current){
		...
		var pro=$("#pro").val();
		$.ajax( {
			url : 'product_manyTermCheck.action',
			dataType : 'json',
			data : {
				sid:sortid,
				pname:pro,
				...
				if(mydata.length>1){
					$.each(mydata,function(index,xx){
						if(xx.PName!=null){
							...
							+xx.PArea+'</td><td>'+xx.TSort.SName+'</td><td>'+xx.PMl+'</td><td>'						
							+(xx.PIs==1?'在售':'已下架')+'</td></tr>';
							$("#proTbody").append(str);
						}else{
							$current=xx.current;
							$up=xx.up;
