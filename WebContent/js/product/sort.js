...
function getAll(current,sis){
	$.ajax({
		url:'sort_allTSort.action',
		dataType:'json',
		data:{current:current,sis:sis},
		type:'post',
		success:function(mydata){
			...
			$.each(mydata,function(index,xx){
				if(xx.SId!=null){
					var str='<tr>';
					if($userLevel!=-1)
						str+='<td>'+xx.SId+'</td><td>'+xx.SName+'</td><td>';
					else
						str+='<td>'+xx.SId+'</td><td class="uppitem">'+xx.SName+'</td><td class="uppitem">';
					str+=(xx.SRemark==null?'暂无备注':xx.SRemark)+'</td><td>';
					if($userLevel!=-1){
...
function delSort(){
	$(document).on("click",".delItem",function() {
		var sid=$(this).parent().parent().find("td:eq(0)").text().trim();
