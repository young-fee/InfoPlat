...
function getFirstAll(current){
	$.ajax({
		url:'product_allTProduct.action',
		dataType:'json',
...
function getSecondAll(current){
	$.ajax({
		url:'product_allNameselect.action',
		dataType:'json',
			...
			if(mydata.length>1){
				$.each(mydata,function(index,xx){
					if(xx.PName!=null){
						...
						var str='<tr><td>'+xx.PId+'</td><td>'
						+'<a class="showmaney"><i class="fa fa-link fa-lg"></i>'+xx.PName+'</a></td><td>'
						+xx.PArea+'</td><td>'+xx.PMl+'</td><td>';
						if($userLevel!=-1){
...
function getThirdAll(current){
	$.ajax({
		url:'productyear_allTProductyear.action',
		dataType:'json',
			...
			if(mydata.length>1){
				$.each(mydata,function(index,xx){
					if(xx.YName!=null){
						var str="";
						if($userLevel!=-1){
							str='<tr><td>'+xx.YName+'</td><td>￥'
						+xx.YPrice+'</td></tr>';
						}else{
							str='<tr><td>'+xx.YName+'</td><td class="upmoney">￥'
						+xx.YPrice+'</td></tr>';
						}
						//<tr><td>1</td><td>工工业酒精</td><td>武汉</td><td>ML</td><td>修修改信息</td><td>更更新价格</td><td>下下架</td></tr>
						$("#proTbody").append(str);
			...
			if(mydata.length>1){
				$.each(mydata,function(index,xx){
					if(xx.PName!=null){
						//<tr><td>1</td><td>工工业酒精</td><td>武汉</td><td>ML</td><td>修修改信息</td><td>更更新价格</td><td>下下架</td></tr>
						var str='<tr><td>'+xx.PId+'</td><td>';
						str+='<a class="showmaney"><i class="fa fa-link fa-lg"></i>'+xx.PName+'</a></td><td>';
						str+=xx.TSort.SName+'</td><td>'+xx.PArea+'</td><td>'+xx.PMl+'</td><td>';
						if(xx.TSort.SIs==0)
